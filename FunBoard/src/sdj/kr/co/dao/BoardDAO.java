...
public class BoardDAO {
	...
	public PageTO query(int curPage,String query) {

		String sql = "select * from (select rownum rnum, num, title, writer, " + "writeday, readcnt, repIndent from"
				+ "(select * from BOARD where title like ? order by repRoot desc, repStep asc))" + "where rnum >= ? and rnum <= ?";
		...
		try {
			conn = dataFactory.getConnection();
			int amount = getAmount(conn,query);
			to.setAmount(amount);
