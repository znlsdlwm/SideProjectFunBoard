...
public class MemberDAO {
	...
	public void membership(MemberDTO dto) {
		Connection conn = null;
		PreparedStatement pstmt = null;
	...
	public List<BoardDTO> list() {
		...
		try {
			...
			while (rs.next()) {
				...
				list.add(new BoardDTO(r_num, r_writer, r_title, null, r_writeday, r_readcnt, r_repRoot, r_repStep, r_repIndent));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}finally {
	...
	public BoardDTO read(int r_num) {
		BoardDTO dto = null;
		Connection conn = null;
		PreparedStatement pstmt = null;
		...
		try {
			...
			if (rs.next()) {
				...
				dto = new BoardDTO(r_num, r_writer, r_title, r_content, r_writeday, r_readcnt+1, 0, 0, 0);
				isOk = true;
	...
	public BoardDTO updateUI(int r_num) {
		BoardDTO dto = null;
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "select * from board_rjs where r_num = ?";
		...
		try {
			...
			if (rs.next()) {
				...
				dto = new BoardDTO(r_num, r_writer, r_title, r_content, null, 0, r_repRoot, r_repStep, r_repIndent);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}finally {
	...
	public List<MemberDTO> selectAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				list.add(new MemberDTO(r_id, r_name, r_age, null));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}finally {
	...
	try {
		...
		while (rs.next()) {
			...
			BoardDTO dto = new BoardDTO(r_num, r_writer, 
	...
	public MemberDTO selectById(String r_id) {
		MemberDTO dto = null;
		Connection conn = null;
		PreparedStatement pstmt = null;
		...
		try {
			...
			if (rs.next()) {
				...
				dto = new MemberDTO(r_id, r_name, r_age, null);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}finally {
	...
	public void membershipupdate(MemberDTO dto) {
		Connection conn = null;
		PreparedStatement pstmt = null;
	...
	public MemberDTO membershipupdateui(String id) {
	...
	public PageTO searchList(int curPage, String keyField, String keyWord) {
	      ...
	      try {
	         ...
	         while (rs.next()) {
	            ...
	            int r_readcnt = rs.getInt("r_readcnt");
	            int r_repIndent = rs.getInt("r_repIndent");
	            BoardDTO dto = new BoardDTO(r_num, r_writer, r_title, null, r_writeday, r_readcnt, 0 , 0, r_repIndent);
	            list.add(dto);
	         }
